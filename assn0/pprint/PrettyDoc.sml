...
sig
  ...
  val space: doc
  val softspace: doc
  val make_flattened: doc -> doc
  val group: doc -> doc

  val parensAround: doc -> doc
  val spaces: int -> doc
...
struct
  ...
  fun flatten doc =
    let
      ...
      fun loop doc =
        case doc of
          Empty => (false, Empty, 0, false)
        | Space keepSpace => (keepSpace, Empty, 0, keepSpace)
        | Text str => (false, Text str, String.size str, false)
      ...
      and loopBeside (d1, d2) =
        let
          ...
          val (l, m, sz, r) =
            ...
            | (_, false, _) => (false, Beside (flat1, flat2), sz1 + sz2, false)
            | _ =>
                ( false
  ...
  fun make_flattened input =
    let val (_, result, _, _) = flatten input
    in result
    end
  ...
  fun pretty {ribbonFrac, maxWidth} inputDoc =
    let
      ...
      fun layout (lnStart, col, acc) doc : int * int * (string list) =
        ...
        | Beside (doc1, doc2) => layout (layout (lnStart, col, acc) doc1) doc2
        | Above (_, doc1, doc2) =>
            let
